...
disp('getting images')
load('konkle_img.mat','konkle_imgs');
imsize = 256;
tt_img = nan(imsize,imsize,3,length(konkle_imgs));
ttex_img = nan(imsize,imsize,3,length(konkle_imgs));
for tt=1:length(konkle_imgs)
    img = imresize(konkle_imgs(tt).img,[imsize imsize]);
    tt_img(:,:,:,tt) = double(repmat(img,1,1,3));
    tt_params_st(tt) = getPixelFeatures(tt_img(:,:,:,tt),tt); %#ok<SAGROW>

    img = imresize(konkle_imgs(tt).tex_img,[imsize imsize]);
...
tt_params = [[konkle_imgs.big]' [konkle_imgs.animal]' tt_params_mat];
...
ttex_params = [[konkle_imgs.big]' [konkle_imgs.animal]' ttex_params_mat];
...
save('cache_lily_fast.mat','tt_randabl','tt_signeg')
...
save('cache_lily_fast.mat','ttex_randabl','ttex_signeg','-append')

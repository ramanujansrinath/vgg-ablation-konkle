...
if exist('cached_images.mat','file')
    ...
    % find a set of orthogonal bases for pixel features,
    % and prepend the high-level semantic labels (size/animacy) stored
    % in the image metadata as the first two feature columns.
    beta = pca([tt_params_mat; ttex_params_mat],'NumComponents',10);
    tt_params = [[texform_img.big]' [texform_img.animal]' tt_params_mat * beta];
    ttex_params = [[texform_img.big]' [texform_img.animal]' ttex_params_mat * beta];
    ...
    pmin = repmat(min([tt_params; ttex_params],[],'omitnan'),size(tt_params,1),1);
    pmax = repmat(max([tt_params; ttex_params],[],'omitnan'),size(tt_params,1),1);
    tt_params = (tt_params-pmin)./(pmax-pmin);
    ttex_params = (ttex_params-pmin)./(pmax-pmin);

    good_images = ~isnan(tt_params(:,end)+ttex_params(:,end));
    
    save('cached_images.mat','tt_img','tt_params','ttex_img','ttex_params','good_images')
end
...
if input('Redo ablation analysis? (1/0): ')
    ...
    for ll=1:length(layers)
        ...
        [tt_randabl{ll},tt_signeg{ll}] = getabl(net,layers(ll),tt_img,tt_params,good_images);
    end
    ...
    for ll=1:length(layers)
        disp(['analyzing layer' num2str(layers(ll))])
        [ttex_randabl{ll},ttex_signeg{ll}] = getabl(net,layers(ll),ttex_img,ttex_params,good_images);
    end
...
function [tt_randabl,signegabl] = getabl(net,layerNum,tt_img,tt_params,good_images)
    tt_img = tt_img(:,:,:,good_images);
    tt_params = tt_params(good_images,:);
